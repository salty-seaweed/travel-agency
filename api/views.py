...
class ResortViewSet(viewsets.ModelViewSet):
    ...
    ordering_fields = ['name', 'price_per_night_from', 'star_rating', 'display_order', 'created_at']
    ...
    def get_queryset(self):
        ...
        # Filter by atoll if provided
        atoll = self.request.query_params.get('atoll')
        if atoll:
        ...
        if min_price:
            queryset = queryset.filter(price_per_night_from__gte=min_price)
        if max_price:
            queryset = queryset.filter(price_per_night_from__lte=max_price)
        
        # Filter by special features
        adults_only = self.request.query_params.get('adults_only')
        if adults_only == 'true':
            queryset = queryset.filter(is_adults_only=True)
        
        family_friendly = self.request.query_params.get('family_friendly')
        if family_friendly == 'true':
            queryset = queryset.filter(is_family_friendly=True)
        
        honeymoon = self.request.query_params.get('honeymoon')
        if honeymoon == 'true':
            queryset = queryset.filter(is_honeymoon_special=True)
...
class ResortReviewViewSet(viewsets.ModelViewSet):
...
class ResortAmenityViewSet(viewsets.ModelViewSet):
...
@api_view(['GET'])
@permission_classes([AllowAny])
def resorts_by_category(request):
    """Get resorts grouped by category"""
    try:
        categories = Resort.RESORT_CATEGORIES
        result = {}
...
@api_view(['GET'])
@permission_classes([AllowAny])
def resorts_by_atoll(request):
    """Get resorts grouped by atoll"""
    try:
        atolls = Resort.objects.filter(is_active=True).values_list('atoll', flat=True).distinct()
        result = {}
        
        for atoll in atolls:
            if atoll:  # Skip empty atolls
                resorts = Resort.objects.filter(
                    atoll=atoll,
                    is_active=True
                ).select_related('location', 'language').prefetch_related('images', 'reviews')
