#!/usr/bin/env python3
"""
Railway-compatible script to add Maldives honeymoon packages to the production database.
This script is optimized for Railway deployment and includes proper error handling.
"""
...
def setup_django_environment():
    """Setup Django environment for Railway"""
    print("🔧 Setting up Django environment for Railway...")
    
    # Use minimal settings for Railway production
...
def create_location_and_destination():
    ...
    print("🏝️ Creating location and destination for Maamigili...")
    
    try:
        from api.models import Location, Destination
        ...
        print(f"❌ Error creating location/destination: {e}")
        return None, None
...
def create_experiences():
    ...
    try:
        ...
        print(f"❌ Error creating experiences: {e}")
        return []
...
def create_package(package_data, location, destination, experiences):
    ...
    try:
        from api.models import (
            Package, PackageDestination, PackageInclusion, 
            PackageActivity
        )
...
def main():
    """Main function to create all Maldives packages on Railway"""
    print("🌺 Starting Maldives Package Creation Script for Railway 🌺")
    print("=" * 70)
    
    # Setup Django environment
    if not setup_django_environment():
        print("❌ Failed to setup Django environment. Exiting.")
        sys.exit(1)
    
    # Create location and destination
    location, destination = create_location_and_destination()
    if not location or not destination:
        print("❌ Failed to create location/destination. Exiting.")
        sys.exit(1)
    
    # Create experiences
    experiences = create_experiences()
    if not experiences:
        print("⚠️  No experiences created, but continuing...")
    ...
    for package_data in packages_data:
        ...
        if package:
            ...
            failed_packages.append(package_data['name'])
    ...
    if failed_packages:
        print(f"\n❌ Failed to create {len(failed_packages)} packages:")
    ...
    print("   • All packages include airport pickup & speedboat transfers")
    ...
    if created_packages:
        print("\n🚀 Packages are now live on Railway!")
        print("🌐 Check your Railway deployment to see the new packages")
    else:
        print("\n⚠️  No packages were created. Check the error messages above.")
        sys.exit(1)
