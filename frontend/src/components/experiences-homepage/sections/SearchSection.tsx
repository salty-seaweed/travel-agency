...
export const ExperiencesSearchSection: React.FC = () => {
  ...
  const allDestinations = destinationsData?.results?.map(dest => dest.name) || ['Malé', 'Maafushi', 'Hulhumalé', 'Thulusdhoo', 'Dhiffushi', 'Guraidhoo'];
  ...
  }, [searchData.destination, allDestinations]);
  ...
  const handleSearch = () => {
    ...
    // Search both packages and resorts
    if (searchData.destination) {
      // Try packages first, then resorts
...
Please help me plan this perfect trip!`;
  ...
  return (
    <Box bg={bgColor} py={12} borderBottom="1px solid" borderColor={borderColor}>
      <Container maxW="7xl">
        <VStack spacing={8}>
          {/* Hide the "Find Your Maldives Packages" section as requested */}
                          ...
                          {showDestinationDropdown && filteredDestinations.length > 0 && (
                            ...
                            >
                              {filteredDestinations.map((destination) => (
                                ...
                                >
                                  <Text fontSize="sm" color={textColor}>{destination}</Text>
                                </Box>
                              ))}
                            </Box>
                          )}
