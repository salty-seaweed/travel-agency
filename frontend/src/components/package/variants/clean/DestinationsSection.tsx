...
export function DestinationsSection({ destinations, hideHeader }: DestinationsSectionProps) {
  ...
  return (
    <VStack align="stretch" spacing={6}>
      <GoogleMap destinations={destinations} height={280} />
      <VStack align="stretch" spacing={2}>
        {destinations.map((dest, index) => {
          const name = dest.location?.island || `Destination ${index + 1}`;
          return (
            <HStack key={dest.id || index} justify="space-between" py={2} borderBottom="1px solid" borderColor="gray.200">
      ...
      {destinations.some((d) => d.description) && (
        <VStack align="stretch" spacing={2}>
          {destinations.map((dest, index) =>
            dest.description ? (
              <Box key={index}>
                <Text fontSize="sm" fontWeight="medium" color={theme.colors.textPrimary}>
                  {dest.location?.island || `Destination ${index + 1}`}
                </Text>
                <Text fontSize="sm" color={theme.colors.textSecondary} mt={1}>
                  {dest.description}
                </Text>
              </Box>
            ) : null
