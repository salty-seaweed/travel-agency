...
export function ReviewSystem({ propertyId, propertyName, onReviewSubmitted }: ReviewSystemProps) {
  ...
  const fetchReviews = async () => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/reviews/?property=${propertyId}`);
      if (response.ok) {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showError(errorData.message || 'Failed to submit review');
      }
    } catch (error) {
